...
const selectFile = () => {
  fireEvent.click(screen.getByRole("button", { name: /choose from device/i }));
  const input = screen.getByTestId("receipt-gallery-input") as HTMLInputElement;
...
describe("ReceiptScanDrawer", () => {
  ...
  it("uses a device-library input without the camera capture attribute", () => {
    render(<ReceiptScanDrawer open onOpenChange={vi.fn()} />);
    ...
    expect(screen.queryByTestId("receipt-file-input")).not.toBeInTheDocument();
    expect(
