...
export type DashboardPayerTotal = {
  total: number;
  totals: number[];
};
...
export type DashboardMonthlySummary = {
  activeMonth: string;
  payerOptions: string[];
  totalsByPayer: Record<string, DashboardPayerTotal>;
};
...
export const getDashboardMonthlySummary = async (
  month?: string
  ...
  const payerTotals = Array.from(totalsByPayer.entries()).reduce<
    Record<string, DashboardPayerTotal>
  >((acc, [payer, amounts]) => {
    acc[payer] = {
  ...
  return {
    ...
    payerOptions: Object.keys(totalsByPayerForDisplay),
    totalsByPayer: totalsByPayerForDisplay,
  };
};
