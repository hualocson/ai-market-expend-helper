...
| Manual expense form    | `src/components/ManualExpenseForm.tsx`      | AI budget suggestion returns no match       | `warning()`         | implemented | Triggered after a current `no_match` response leaves the budget unchanged.            |
| AI quick entry         | `src/components/AIQuickEntry.tsx`           | Parse auto-saves a trusted expense          | `success()`         | implemented | Triggered after the expense mutation resolves and the saved entry is visible.         |
| AI quick entry         | `src/components/AIQuickEntry.tsx`           | Parse needs user review                     | `warning()`         | implemented | Triggered when the parsed entry is routed to the review queue.                        |
| AI quick entry         | `src/components/AIQuickEntry.tsx`           | Parse or create request fails               | `error()`           | implemented | Triggered when the entry is moved to review because a request failed.                 |
| AI quick entry         | `src/components/AIQuickEntry.tsx`           | Composer submits a non-empty expense        | `impact("medium")`  | implemented | Triggered before the natural-language expense is queued for parsing.                  |
...
| Quick expense sheet    | `src/components/QuickExpenseSheet.tsx`      | AI budget suggestion applies a match        | `success()`         | implemented | Triggered after a current high/medium-confidence match preselects a budget.           |
| Quick expense sheet    | `src/components/QuickExpenseSheet.tsx`      | AI budget suggestion returns no match       | `error()`           | implemented | Triggered after a current `no_match` response leaves the budget unchanged.            |
...
| Bottom navigation      | `src/components/BottomNav.tsx`              | AI quick entry button is clicked            | `impact("medium")`  | implemented | Triggered before opening the app-wide AI quick entry drawer.                          |
